            ...
            GestureDetector(
              onTap: () async {
                final confirmed = await showDialog<bool>(
                  ...
                  builder: (ctx) => AlertDialog(
                    ...
                    title: Text(
                      AppStrings.isRu ? 'Удалить аккаунт?' : 'Akkauntni o\'chirish?',
                      style: theme.textTheme.titleMedium,
                    ),
                    content: Text(
                      AppStrings.isRu
                          ? 'Это действие необратимо. Все ваши данные будут удалены.'
                          : 'Bu qaytarilmas harakat. Barcha ma\'lumotlaringiz o\'chiriladi.',
                      style: theme.textTheme.bodyMedium,
                    ),
                    actions: [
                      TextButton(
                        onPressed: () => Navigator.pop(ctx, false),
                        child: Text(AppStrings.isRu ? 'Отмена' : 'Bekor qilish'),
                      ),
                      TextButton(
                        onPressed: () => Navigator.pop(ctx, true),
                        child: Text(
                          AppStrings.isRu ? 'Удалить' : 'O\'chirish',
              ...
              child: Container(
                ...
                child: Row(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    Icon(Icons.delete_forever_rounded, color: AppColors.error.withOpacity(0.7), size: 20),
                    const SizedBox(width: 8),
                    Text(
                      AppStrings.isRu ? 'Удалить аккаунт' : 'Akkauntni o\'chirish',
                      style: TextStyle(color: AppColors.error.withOpacity(0.7), fontWeight: FontWeight.w600, fontSize: 15),
                    ),
                  ],
                ),
              ),
