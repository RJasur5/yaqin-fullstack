...
class _ChatScreenState extends State<ChatScreen> {
  ...
  void initState() {
    ...
    SharedPreferences.getInstance().then((prefs) {
      prefs.setInt('activeChatOrderId', widget.order['id']);
    });
  ...
  void _openProfile(BuildContext context, bool isClient) {
    final String? otherRole = widget.order['other_user_role'];
    
    // Explicit target IDs from ChatListScreen
    final rawOtherMasterId = widget.order['other_master_id'];
    final rawOtherClientId = widget.order['other_user_id'];
    
    // Fallback target IDs from OrderResponse (e.g. from MyOrdersScreen / AcceptedOrdersScreen)
    final rawOrderMasterId = widget.order['master_id'];
    final rawOrderClientId = widget.order['client_id'];

    int? targetMasterId = rawOtherMasterId != null 
        ? int.tryParse(rawOtherMasterId.toString()) 
        : (rawOrderMasterId != null ? int.tryParse(rawOrderMasterId.toString()) : null);
        
    int? targetClientId = rawOtherClientId != null 
        ? int.tryParse(rawOtherClientId.toString()) 
        : (rawOrderClientId != null ? int.tryParse(rawOrderClientId.toString()) : null);

    // If backend provided 'other_user_role', use it directly:
    if (otherRole == 'master') {
      if (targetMasterId != null) {
        Navigator.push(context, MaterialPageRoute(builder: (_) => MasterDetailScreen(apiService: widget.apiService, masterId: targetMasterId)));
      } else if (targetClientId != null) {
        Navigator.push(context, MaterialPageRoute(builder: (_) => ClientProfileScreen(clientId: targetClientId, apiService: widget.apiService)));
      }
      return;
    } else if (otherRole == 'client') {
    ...
    // Fallback: If no role is provided, rely on isClient boolean
    if (isClient) {
      // I am the Client (Employer). I should see the Master profile.
      if (targetMasterId != null) {
        Navigator.push(context, MaterialPageRoute(builder: (_) => MasterDetailScreen(apiService: widget.apiService, masterId: targetMasterId)));
      } else if (targetClientId != null) {
        Navigator.push(context, MaterialPageRoute(builder: (_) => ClientProfileScreen(clientId: targetClientId, apiService: widget.apiService)));
      ...
      // I am the Master (Worker). I should see the Client profile.
      if (targetClientId != null) {
        Navigator.push(context, MaterialPageRoute(builder: (_) => ClientProfileScreen(clientId: targetClientId, apiService: widget.apiService)));
      }
    }
  ...
  Widget build(BuildContext context) {
    ...
    final isClient = widget.order['client_id'] != null && widget.currentUserId.toString() == widget.order['client_id'].toString();
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => _openProfile(context, isClient),
          ),
          const SizedBox(width: 8),
        ],
        title: GestureDetector(
          behavior: HitTestBehavior.opaque,
          onTap: () => _openProfile(context, isClient),
          child: Row(
            children: [
              GestureDetector(
                onTap: () => _openProfile(context, isClient),
  ...
  void dispose() {
    ...
    if (NotificationService.activeChatOrderId == widget.order['id']) {
      ...
      SharedPreferences.getInstance().then((prefs) {
        prefs.remove('activeChatOrderId');
      });
    }
