      ...
      child: Scaffold(
        ...
        bottomNavigationBar: Container(
          ...
          child: BottomNavigationBar(
            currentIndex: 4, // "Profile" is conceptually the parent
            type: BottomNavigationBarType.fixed,
            selectedItemColor: Theme.of(context).primaryColor,
            unselectedItemColor: Theme.of(context).hintColor,
            onTap: (i) {
              Navigator.popUntil(context, (route) => route.settings.name == '/home');
              HomeScreen.switchToTab?.call(i);
            },
            items: [
              BottomNavigationBarItem(icon: const Icon(Icons.home_rounded), label: AppStrings.home),
              BottomNavigationBarItem(icon: const Icon(Icons.search_rounded), label: AppStrings.search),
              BottomNavigationBarItem(icon: const Icon(Icons.chat_bubble_outline_rounded), label: AppStrings.isRu ? "Чаты" : "Chatlar"),
              BottomNavigationBarItem(icon: const Icon(Icons.assignment_rounded), label: AppStrings.orders),
              BottomNavigationBarItem(icon: const Icon(Icons.person_rounded), label: AppStrings.profile),
            ],
          ),
        ),
      ),
      ...
      if (!isStillOpen) {
        ...
        if (days > 0) {
          countdownText = AppStrings.isRu
              ...
              : 'Qoldi: ${days} kun ${hours} soat';
        } else if (hours > 0) {
          countdownText = AppStrings.isRu
              ? 'Осталось: ${hours}ч ${min}м'
              : 'Qoldi: ${hours} soat ${min} daq';
        } else {
          countdownText = AppStrings.isRu
              ? 'Осталось: ${min}м ${sec}с'
              : 'Qoldi: ${min} daq ${sec} son';
        }
      }
    }
